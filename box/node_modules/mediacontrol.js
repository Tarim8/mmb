...
function Flower( petals, stalk ) {
	...
	if( _.isString( petal ) && petal.match( /\*/ ) ) {
	    me.wild.push( '^' + petal.replace( /\./g, '\\.' ).replace( /\*/g, '.*' ) +'$' );
	} else {
...
Flower.prototype.synth = function( commands ) {
	...
	if( _.isFunction( petal ) ) {
	    petal( flower.stalk );
            ...
            // for a.b.c.thing lookup b.c.type then c.type
            var petal_ = petal + '.type';
            while( (petal_ = petal_.replace( /[^\.]*\./, '' )) !== 'type' ) {
                var commandFlower = commands.lookup( petal_ );
                if( commandFlower ) {
...
Bed.prototype.createFlower = function( src, options ) {
    ...
    if( options.move ) {
        fs.move( src, pa.join( dirName, destName ), function( err ) {
            if( err ) {
        ...
        fs.copySync( src, pa.join( dirName, destName ) );
        me.fsAdd( dirName );
        me.findStalkIndex( dirName );
        options.success && options.success();
    }
...
function createDeviceType( device, type, startFunc, stopFunc ) {
	...
	device.channel.play( new device[type]( path, options ) );
    };	
