...
function recordStop() {
	...
	currentTrack = memories.findStalkIndex( recordDir );
    }
};
...
action.play = function( path, options ) {
    ...
    if( options.move === 'play' ) {	// or last button press less than few seconds ago
	move = channel.playing ? 1 : 0;
    } else if( move === NaN ) {
	move = 0;
    }

    currentTrack = mod( currentTrack + move, memories.flowers.length );
    commands.synthesize( memories.flowers[currentTrack] );
};

action.edittag = function( path, options ) {
    if( options.action === 'attach' ) {
	if( memories.lookup( Tag ) ) {
	    commands.synthesize( universal.lookup( 'tagattached.err.tag' ) );

	} else {
	    fs.closeSync( fs.openSync( pa.join( memories.flowers[currentTrack].stalk, Tag ), 'a' ) );
...
function lookupMemory( tag ) {
    commands.synthesize( currentMenu.lookup( tag ) ) ||
    commands.synthesize( universal.lookup( tag ) ) ||
    commands.synthesize( memories.lookup( tag ) ) ||
    commands.synthesize( currentMenu.lookup( 'unknown.tag' ) ) ||
