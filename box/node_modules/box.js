...
var recordSession = {};
recordSession.killSession = _.identity;
var recordTmp='/tmp/mcrecord' + process.pid + '.wav';

action.record = function( path, options ) {
    if( options.action === 'stoprecord' ) {
        if( recordSession.leader && recordSession.leader.pid ) {
            log( 'Info record SIGINT' );
            try {
                process.kill( recordSession.leader.pid, 'SIGINT' );
            } catch( err ) {
        ...
        if( recordSession.leader && _.isNull( recordSession.leader.exitCode ) ) {
            log( 'Info record busy' );
            lookupMenu( 'recordbusy.tag' );
            return;
        }
	...
	recordSession = new Session(
		...
		options.audiodriver ? [ '--audiodriver', options.audiodriver ] : [],
		recordTmp
	    ] ),
	    function() {
...
action.copyfiles = function( path, options ) {
    log( 'Info copying' );
    var USBPath = options.path || '/media/usb/';
    var isDir;
    try {
	isDir = fs.statSync( USBPath ).isDirectory();
    } catch( err ) {
	if( err.code !== 'ENOENT' ) throw( err );
	isDir = false;
    }
    ...
    if( isDir ) {
	lookupMenu( 'usbaddmsg.tag' );
	fsAddFiles( USBPath, 0 );
	log( 'Info copied' );
    }
};

action.backupfiles = function( path, options ) {
    log( 'Info backup' );
    var USBPath = options.path || '/media/usb/';
    var isDir;
    try {
    ...
    if( isDir ) {
	lookupMenu( 'usbbackupmsg.tag' );
	var destLog = pa.join( USBPath, 'box.log' );
	try {
	...
	fs.copySync( pa.join( collectionPath, '..', '..', 'logs', 'box.log' ), destLog );
    
	fs.readdirSync( collectionPath ).forEach( function( dir ) {
	    var destDir = pa.join( USBPath, dir );
	    try {
...
action.unmount = function( path, options ) {
    log( 'Info unmount' );
    var USBPath = options.path || '/media/usb/';
    new Session( 'sudo', [ 'umount', USBPath ] );
};
