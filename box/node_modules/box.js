...
function recordDone() {
    log( 'Info record done ' + this.exitCode );
    recordSession = false;

    if( this.exitCode === 0 ) {
	var recordName = Date.now().toString();
	var recordDir = pa.join( collectionPath, recordName );
	fs.mkdirSync( recordDir );
	// should use mv for cross volume
	fs.renameSync( recordTmp, pa.join( recordDir, recordName + '.wav' ) );

	collection.fsAdd( recordDir );
	collection.findStalkIndex( recordDir );
	lookupMemory( 'recordfinished.tag' );

    } else {
	lookupMenu( 'recordfailed.tag' );
    }   
};

action.record = function( path, options ) {
    recordStop();
    if( options.onAir && !recordSession ) {
