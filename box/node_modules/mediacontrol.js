...
// Flower.wild			[ 'rfid*.tag' ]
// Flower.stalk			'/dir/bbb/'

function Flower( petals, stalk ) {
    var me = this;
    if( stalk ) me.stalk = stalk;
    me.petals = [];
    me.wild = [];

    petals.forEach( function( petal ) {
	if( _.isString( petal ) && petal.match( /\*/ ) ) {
	    me.wild.push( '^' + petal.replace( /\./, '\\.' ).replace( /\*/, '.*' ) +'$' );
	} else {
	    me.petals.push( petal );
	}
    } );
...
Flower.prototype.lookup = function( petal ) {
    if( this.petals.indexOf( petal ) != -1 ) return this;
    for( var wildIndex in this.wild ) {
	if( petal.match( this.wild[wildIndex] ) ) return this;
    }
    return false;
};
...
//
// start( media )	start the media object
//
Channel.prototype.start = function( media ) {
    this.setBrightness( this.fullIntensity );
    this.next = false;

    if( media.name != this.media.name || !this.running ) {
	var me = this;
	log( 'Channel start ' + media.name );
	if( me.running && (media.fadeLast || me.media.fadeMe) ) {
...
//
// device constructor generator
// used for media and recorder
//
function createDeviceType( device, type, startFunc, stopFunc ) {
    device[type] = function( name, options ) {
	this.name = name;
	copyAttributes( options, this );
    };

    action[type] = function( path, options ) {
	device.channel.start( new device[type]( path, options ) );
    };	

    device[type].prototype.start = startFunc;

    if( stopFunc ) {
	device[type].prototype.stop = stopFunc;

    } else {
	device[type].prototype.stop = function() {
...
createDeviceType(
	...
	if( this.action === "record" ) {
		...
		'voicerec',
		_.flatten( [
		    this.audiodev ? [ '--audiodev', this.audiodev ] : [],
		    recorder.tmpFile
		] ),
		recorder.callback,
		{}
    ...
    function() {
	this.session && this.session.killSession( 'SIGINT' );
	this.session = false;
    }
