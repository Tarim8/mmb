...
var selectionsPath = pa.join( dbDir, 'selections' );
var selections = {};
fs.readdirSync( selectionsPath ).forEach( function( dir ) {
    selections[dir] = (new mc.Bed( pa.join( selectionsPath, dir ) )).flowers;
} );
...
currentMenu.flowers = selections.start;
...
function lookupMemory( tag ) {
	...
	mc.marks.select.forEach( function( name ) {
	    if( selections[name] ) {
		selections[name].forEach( function( flower ) {
