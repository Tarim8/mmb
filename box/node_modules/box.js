...
fs.readdirSync( menusPath ).forEach( function( dir ) {
    menus[dir] = new mc.Bed( pa.join( menusPath, dir ) );
} );
...
var currentMenu = new mc.Bed( menus.play );

function lookupMemory( tag ) {
    commands.synthesize( currentMenu.lookup( tag ) ) ||
    // currentMenu.synthesize( currentMenu.lookup( tag ) ) ||
    currentMenu.synthesize( 'unknown.tag', { tag: tag } ) ||
    commands.synthesize( memories.lookup( tag ) ) ||
    commands.synthesise( 'unknown.tag', { tag: tag } ) ||
    log( 'Tag unknown ' + tag );
	...
	mc.marks.menu.forEach( function( name ) {
	    if( menus.names[name] ) {
		currentMenu.flowers.push( menus.names[name] );
	    } else {
...
function lookupTag( line ) {
    ...
    lookupMemory( pollv[2] + pollv[0] + '.tag' );
};
