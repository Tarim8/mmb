...
// See if a petal exists and return the flower if it does
...
// Bed	{ flowers: [ {petals: ['busy.tag', 'amp.on.out']}, {petals: ['idle.tag', 'amp.off.out']} ]}
//

function Bed( buds, stalk ) {
    this.flowers = [];
    if( _.isArray( buds ) ) this.add( buds, stalk );
    else if( _.isString( buds ) ) this.fsAdd( buds );
};
...
Bed.protoype.synthesize = function( flower ) {
...
media.create(
    ...
    function( callback ) {
	this.session = new Session( 'omxplayer', [this.name], callback );
    }
);
